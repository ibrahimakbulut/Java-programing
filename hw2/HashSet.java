...
/**
* <h1>HashSet<T></h1>
* This class implements Set interface.This is a container.
* <p>
   ...
   */
	...
	public void addAll(Collection<T> c){
			...
			if(container==null){
				...
				for(int i=0;i<((HashSet<T>)c).used;++i){
					int find=0;
					for(int j=0;j<used;++j){
						if(((HashSet<T>)c).container[i].equals(container[j]))
							find=1;
					}
				...
				if(find!=1){
					if(used>=capacity){
						...
						temp[used]=((HashSet<T>)c).container[i];
						used+=1;
						capacity+=10;
						container=temp;
					}
					else{
					container[used]=((HashSet<T>)c).container[i];
					used+=1;
					}
				}
